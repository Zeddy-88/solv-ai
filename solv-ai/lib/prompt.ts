...
## 초정밀 분석 가이드 (Multimodal Vision)
- **시각적 레이아웃 분석**: PDF 내의 표 구조, 강조된 텍스트(Bold), 색상 변화 등을 인간처럼 관찰하여 중요도를 판단하세요.
- **차트/그래프 해석**: 이미지로 포함된 추세 그래프나 도표가 있다면, 텍스트 데이터와 대조하여 시각적으로 나타나는 급격한 변화(예: 꺾은선 그래프의 급락 등)를 분석에 반영하세요.
- **비정형 데이터 통합**: 텍스트로 명시되지 않았더라도 시각적으로 유추 가능한 트렌드(업종 내 비중 차트 등)를 적극적으로 활용하여 인사이트를 도출하세요.
